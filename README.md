...
> 💡 **[AIPOCH Open-Science v0.31.0 released](https://github.com/aipoch/open-science/releases/latest)** _(last updated September 2026)_. AIPOCH Open-Science v0.31.0 lets you fork a writable session carrying its complete research history — conversation branches, notebook records, artifacts, literature, and bookmarks — while imported sessions stay read-only. The product now presents itself consistently as Open-Science, with existing installations and all research data preserved. Session plans recover their context after agent context reconstruction, provider connections are validated before they are saved, the gnomAD connector can report population-level frequencies, and branched sessions show a continuation divider back to their source. Windows R runs in standard mode without protected-mode setup, and connectors, dialogs, and permission handling are tightened throughout. A session information card shows each session's number, title, description, source, timestamps, and message and artifact counts with a pin control, and credential prompts now link to the official API key pages. See the [latest release notes](https://github.com/aipoch/open-science/releases/latest) for full details.
